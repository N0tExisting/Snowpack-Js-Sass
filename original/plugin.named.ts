...
module.exports = function sassPlugin(snowpackConfig: SnowpackUserConfig,  settings): SnowpackPlugin {
  const {root} = snowpackConfig || {};
  const { native, compilerOptions = {} } = settings;
  ...
  return {
     ...
     * @private
     */
    _markImportersAsChanged(filePath: string) {
